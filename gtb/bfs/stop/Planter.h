...
class Planter {
	public:
		const vector<double>& run(const vector<ID>& train_targets, const vector<ID>& test_targets, const size_t tree_count);
	...
	private:
		...
		void grow(const vector<ID>& train_targets, const vector<ID>& test_targets, const size_t tree_count, size_t depth);
